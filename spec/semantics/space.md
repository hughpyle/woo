...
3. **Assign `seq` and append to log.** `seq = this:append(message)` (inherited from `$sequenced_log`; see [sequenced-log.md §SL2](sequenced-log.md#sl2-the-native-verbs)). The native verb atomically allocates the seq, increments `next_seq`, and durably persists `(seq, message)`. **This is the commit point for the message having been accepted.** The row's outcome (success or rolled-back) is recorded separately by step 6/8 — see [reference/cloudflare.md §R3.2](../reference/cloudflare.md#r32-two-phase-log-writes) for the storage-layer split. If the storage layer fails to commit this step, the call is rejected pre-sequence with `E_STORAGE` and `next_seq` does not advance — see [failures.md §F6](failures.md#f6-storage-and-persistence-failures). The runtime never reaches "seq advanced but message not in the log."
