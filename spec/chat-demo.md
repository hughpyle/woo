...
The third demo. A canonical MOO surface — rooms, presence, talk, emote, tell — built as a feature-object composition rather than a `$space` subclass. Sits alongside dubspace/taskspace/IDE; can also embed inside them.
...
The chat verbs use the **direct live interaction** pattern from [core.md §C13](semantics/core.md#c13-call-discipline). Each row classifies one verb across the two axes from [core.md §C12.1](semantics/core.md#c121-two-orthogonal-axes); observation durability follows from the route automatically.
...
Because every row routes directly, every observation these verbs emit is live-only by [events.md §12.6](semantics/events.md#126-observation-durability-follows-invocation-route): pushed to subscribers, never stored. A late-joining client sees no scrollback. This matches MOO's `notify()` semantics.
...
> Being a `$space` does not mean every verb on the object is sequenced ([core.md §C12](semantics/core.md#c12-direct-messages-vs-space-mediated-messages)). A `$chatroom` is a `$space` and a feature consumer; chat verbs run as direct calls and never enter the room's sequence log. Saying something does not advance `next_seq`.

**Logged variant (opt-in).** A world that wants auditable chat picks one of:

- **Sequenced via `$space:call`.** Authors call `$chatroom:call({verb: "say", args: ["hi"]})` instead of `$chatroom:say("hi")`. The call is now sequenced; the verb body's `emit` lands in the resulting `applied` frame's observations and is replay-visible per [events.md §12.6](semantics/events.md#126-observation-durability-follows-invocation-route).
...
A feature object (per [features.md](semantics/features.md)) carrying the chat verbs. Attached to any `$space` that wants to act as a room.
...
Live observations flow over the wire as `op: "event"` frames ([wire.md §17.2](protocol/wire.md#172-server--client)) or as SSE `event: event` entries; clients render them in the same chronological feed as applied frames but they are not part of `:replay` history.
...
1. Authenticates as a `$player` (existing flow, [identity.md](semantics/identity.md)).
...
Free-text input goes through `$match:parse_command(text, actor)` per [match.md §MA4](semantics/match.md#ma4-command-parsing). `:command` then explicitly **lowers** the parsed `cmd` map into the right argument shape per verb — the parsed map is not the right call signature for `:say`, `:tell`, etc., so the dispatcher unpacks it:
...
2. **Brings `$match` into use.** The text-to-action pipeline scaffolded in [match.md](semantics/match.md) gets exercised end-to-end. Bugs surface as misparsed commands.
