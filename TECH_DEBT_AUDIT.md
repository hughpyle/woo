...
1. **Implementation code hardcodes object names and catalog-specific verb implementations** (F050, F051). `world.ts` references `the_dubspace` / `the_taskspace` / `the_chatroom` / `slot_1..4` / `channel_1` / `filter_1` / `delay_1` / `drum_1` / `default_scene` in 30+ places, and still registers dubspace native verbs (`save_scene`, `recall_scene`, `set_drum_step`, etc.) directly in `WooWorld`'s constructor. Chat's `$conversational` verbs and Taskspace's task verbs have moved to catalog source in the current worktree, which is the pattern to repeat. Per the project rule: **object-name literals in implementation code are a bad smell and must be prioritized for eradication.** If bundled core objects are required temporarily, their names must live in one core seed data source — not scattered through runtime logic. Bundled catalogs are loaded because of their location, not because the runtime recognizes their names or app roles. This is the single biggest architectural debt.
...
| F050 | Architectural decay | `src/core/world.ts:444-445`, `:473-474`, `:808-870`, `:2118-2342`, `src/worker/persistent-object-do.ts:279-290`, `src/worker/directory-do.ts:20-65` | Critical | L | **Implementation code depends on object-name literals.** 30+ references across runtime/transport code to catalog-installed instance IDs (`the_dubspace`, `slot_1`, `drum_1`, etc.) plus a hardcoded Directory seed table containing both universal and catalog object refs. The project rule is stronger than "do not reference demo instances": object names should live in data (core seed DB, catalog manifests, install records, Directory rows), not in imperative source. | Migrate each call site: catalog projections become catalog/UI adapters (F026), session presence becomes data-driven policy, routing becomes install/object-placement metadata (F019), the dubspace/drum/scene native handlers leave core (F051). Move any truly unavoidable core seed refs into a single seed manifest/imported core DB boundary. Add a CI grep with an allowlist for that one boundary, generated catalog index, tests, and catalog files. |
| F051 | Architectural decay | `src/core/world.ts` native handler registration block, dubspace handler region | Critical | L | **Catalog-specific native verb implementations live in core.** Chat's `$conversational` handlers and Taskspace's task handlers are now source-compiled from their catalog manifests, but `WooWorld` still registers dubspace handlers including `save_scene`, `recall_scene`, `set_drum_step`, `set_tempo`, `start_transport`, `stop_transport`, `preview_control`, and `cursor`. These belong in their respective catalog manifests or a catalog-owned native bundle, not in core. Many also hardcode F050 instance IDs (`slot_1..4`, `drum_1`, `default_scene`). | Repeat the source-catalog pattern where the DSL can express behavior. Where it cannot, expose `world.registerNativeHandler(name, fn)` and have each catalog's installer pass its native handler bundle when installing. Universal handlers (`describe`, `add_feature`, `replay`, `match_object`, `match_verb`, `catalog_registry_*`, `player_moveto`, `guest_on_disfunc`) stay in core. |
...
```ts
// world.ts native registration block — move remaining dubspace native handlers
